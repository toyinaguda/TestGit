...
public class RadixSort{
	...
	static void Distribute(int[] arr, LinkedList[] digits,
		...
		for(int i = 0; i < arr.length; ++i){
			if(digitType.equals("ones")){
				...
				System.out.println(arr[i]%10 + "%"+digits[arr[i] % 10]);
			}else if(digitType.equals("tens")){
				digits[arr[i] / 10].addLast(arr[i]);
				System.out.println(arr[i]/10 + "/"+digits[arr[i] / 10]);
			}/*else{
				digits[arr[i] / 100].addLast(arr[i]);
				System.out.println(arr[i]/100 + "/"+digits[arr[i] / 100]);
			}*/
		}
	}
	...
	public static void main(String[] args){
		...
		Collect(digits,numbers);
		display(numbers);
		/*Distribute(numbers, digits, "100s");
		Collect(digits,numbers);
		display(numbers);*/
